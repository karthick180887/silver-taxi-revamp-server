...
class OneWayReviewScreen extends StatefulWidget {
  const OneWayReviewScreen({
    super.key,
    required this.token,
    ...
    required this.finalAmount, // Note: Recalculated locally, but kept for signature compatibility
    this.phone,
    required this.distance,
    required this.pricePerKm,
    required this.driverBeta,
    // minKm is implicit 130, but we can accept it or ignore validation
  });
  final String token;
  ...
  final double minKm = 130.0; // Hardcoded for One Way
  final String tripType = "One way"; // Hardcoded for One Way

  @override
  State<OneWayReviewScreen> createState() => _OneWayReviewScreenState();
}

class _OneWayReviewScreenState extends State<OneWayReviewScreen> {
      ...
      if (result.success) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('One Way Booking created successfully!'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('One Way Trip'), // Hardcoded Title
      ),

...
export default function OneWayServicePage() {
    const [serviceEnabled, setServiceEnabled] = useState(false);
    const [selectedTab, setSelectedTab] = useState(0);
    const [vehicles, setVehicles] = useState<string[]>(['Mini (3+1)', 'Sedan (4+1)', 'SUV (7+1)', 'Innova (6+1)']);
    ...
    const fetchServiceConfig = async () => {
        try {
            const response = await axios.get(`${API_URL}/v1/services/one-way`, {
    ...
    const handleServiceToggle = async (enabled: boolean) => {
        ...
        try {
            await axios.put(`${API_URL}/v1/services/one-way`, { enabled }, {
                headers: { Authorization: `Bearer ${localStorage.getItem('admin_token')}` },
            });
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <Typography variant="h4" className="font-bold">Outstation - One Way</Typography>
            </div>
            ...
            <Paper className="p-6 space-y-6">
                ...
                <div>
                    <Typography variant="h6" className="font-bold mb-4">One Way Vehicle</Typography>
                    ...
                    <Box className="mt-4 p-4 border border-gray-200 rounded-lg">
                        ...
                        />
                        {/* Vehicle-specific pricing configuration would go here */}
                    </Box>
                </div>
            </Paper>
        </div>
